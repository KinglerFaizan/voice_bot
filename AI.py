...
if 'booking_step' not in st.session_state:
    st.session_state.booking_step = 0  # 0=Start, 1=Dates, 2=Guests, 3=Type, 4=View, 5=Name, 6=Confirm
...
<style>
    /* MAIN BG */
    .stApp {
        background: linear-gradient(to bottom, #050505, #101525), 
...
def process_logic(text):
    ...
    if step == 0:
        # Accept any input to start, or check for keywords
    ...
    # STEP 4: VIEW -> Ask for NAME
        ...
        # Accept whatever view they say, or default
    ...
    # STEP 5: NAME -> Ask for CONFIRM
        ...
        summary = f"Reservation for {b['name']}: {b['type']} room, {b['view']}, {b['guests']} guests, on {b['dates']}. Do you authorize?"
        return summary
...
if not st.session_state.system_ready:
    ...
    with c2:
        ...
        st.markdown("<h1 style='text-align: center; color: #00f3ff; font-size: 60px;'>FRIDAY AI</h1>",
                    unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; letter-spacing: 4px; color: white;'>SYSTEM DEVELOPED BY FAIZAN</p>",
                    unsafe_allow_html=True)
        ...
        if st.button("BOOT SYSTEM PROTOCOLS", use_container_width=True):
            ...
            st.session_state.trigger_audio = intro  # Trigger audio for next render
            st.rerun()