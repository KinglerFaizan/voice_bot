...
if 'chat_history' not in st.session_state:
    st.session_state.chat_history = [
        {"role": "assistant", "content": "System Online. I am Friday. Tap the Orb and state your request."}
    ]
...
<style>
    ...
    /* --- THE ORB (RECORDER) --- */
    /* This styles the mic button to look like a pulsing energy core */
...
# --- 4. AUDIO ENGINE (Hidden & Auto) ---
def play_friday_audio(text):
    """Generates audio and plays it automatically without a player bar."""
    try:
        audio_file = f"friday_{int(time.time())}.mp3"
        tts = gTTS(text=text, lang='en', tld='co.uk')  # British/Friday accent
        tts.save(audio_file)
        ...
        # HTML5 Audio with Autoplay and Display:None
        md = f"""
...
# --- SCENE 1: SYSTEM BOOT (Mandatory for Audio Permissions) ---
if not st.session_state.system_ready:
    ...
    with c2:
        ...
        # This button grants the browser permission to play audio
...
# --- SCENE 2: MAIN INTERFACE ---
    ...
    # --- THE "AUTO-STOP" RECORDER ---
    st.markdown("<br>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1, 1, 1])

    with c2:
        # THIS IS THE KEY COMPONENT
        # just_once=True makes it return immediately after the browser detects silence.
        # You do NOT need to click stop if you pause speaking for 1-2 seconds.
        user_text = speech_to_text(
            language='en',
            start_prompt="●",  # This is the Blue Orb (Idle)
            stop_prompt="🔴",  # This is the Red Orb (Recording)
            just_once=True,
            key='orb'
        )
        st.markdown("<div class='status-text'>TAP ORB • SPEAK • WAIT</div>", unsafe_allow_html=True)
    ...
    # --- AUTO-PLAY WELCOME (First Run Only) ---
    if len(st.session_state.chat_history) == 1 and st.session_state.system_ready:
        # Check if we haven't played the intro yet (using a temp session key)
        if 'intro_played' not in st.session_state:
            play_friday_audio("System Online. I am Friday. How may I assist with your reservation?")
