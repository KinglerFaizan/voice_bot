...
st.set_page_config(
    page_title="FRIDAY | PROTOCOL V.9",
    page_icon="⚛️",
    layout="wide",
...
# Steps: 0=Init, 1=Guests, 2=Type, 3=Confirm, 4=Menu(Exit/New)
...
def process_logic(text):
    ...
    # GLOBAL RESET (Removed "stop" to prevent accidental resets)
    if any(x in text for x in ["reset", "restart"]):
        st.session_state.booking_step = 0
        st.session_state.db['booking'] = {k: None for k in bk}
        return "System reset. Starting Over. Question 1: How many guests?"

    # STEP 0: INITIALIZE -> Ask GUESTS (Q1)
    if step == 0:
        st.session_state.booking_step = 1
        # If user says "2 guests" immediately
        import re
        nums = re.findall(r'\d+', text)
        if nums:
            st.session_state.db['booking']['guests'] = nums[0]
            st.session_state.booking_step = 2
            return f"{nums[0]} guests confirmed. Question 2: Standard, Deluxe, or Presidential Class?"
        return "Hello. I am Friday. Question 1: How many guests will be staying?"

    # STEP 1: GUESTS -> Ask TYPE (Q2)
    elif step == 1:
        import re
        nums = re.findall(r'\d+', text)
        ...
        return "I need a number for the guests. Please state a number."
    ...
    # STEP 2: TYPE -> Ask CONFIRM
        ...
        if "standard" in text:
            ...
            return "Please choose a valid class: Standard, Deluxe, or Presidential."

        # Go directly to confirm
        st.session_state.booking_step = 3
        return f"You selected {st.session_state.db['booking']['type']} for {st.session_state.db['booking']['guests']} guests. Do you authorize this transaction?"

    # STEP 3: CONFIRM -> GO TO MENU (STEP 4)
    elif step == 3:
        if any(x in text for x in ["yes", "confirm", "ok", "sure", "authorize"]):
            st.balloons()
            st.session_state.db['booking']['confirmed'] = True
            st.session_state.booking_step = 4  # Move to Menu Phase
            return "Authorization Verified. Booking Confirmed. Would you like to 'Exit System' or 'Start New'?"

        # If they say No/Cancel
        st.session_state.booking_step = 4  # Move to Menu Phase even if cancelled
        return "Booking Cancelled. Would you like to 'Exit System' or 'Start New'?"

    # STEP 4: MENU PHASE (Exit or New)
    elif step == 4:
        if any(x in text for x in ["exit", "close", "bye", "shut down"]):
            st.session_state.system_ready = False  # Shut down UI
            st.rerun()
            return "Shutting down."
        elif any(x in text for x in ["start", "new", "restart", "begin"]):
            # Reset everything
            st.session_state.booking_step = 1  # Jump to Q1 directly
            st.session_state.db['booking'] = {k: None for k in bk}
            return "Reloading protocols. Question 1: How many guests?"

        return "Command invalid. Please say 'Exit' or 'Start New'."
