...
# Steps: 0=Init, 1=Guests, 2=Type, 3=View, 4=Confirm, 5=ExitChoice
...
def process_logic(text):
    ...
    response = "Input Unrecognized. Please repeat."
    success = False
    ...
    if any(x in text for x in ["reset", "cancel", "stop"]):
        ...
        response = "System reset. Ready for new booking."
        success = True

    # STEP 0: INITIALIZE -> Ask Q1 (GUESTS)
    elif step == 0:
        st.session_state.booking_step = 1
        response = "System Online. I am Friday. Question 1: How many guests will be staying?"
        success = True
    ...
    # STEP 1: GUESTS -> Ask Q2 (TYPE)
        ...
        if nums:
            ...
            response = f"{nums[0]} guests confirmed. Question 2: Would you like Standard, Deluxe, or Presidential Class?"
            success = True
        else:
            response = "I need a number for the guests. Please state a number."
    ...
    # STEP 2: TYPE -> Ask CONFIRM
        ...
        if success:
            ...
            response = f"{st.session_state.db['booking']['type']} room selected. Do you authorize this transaction?"
        else:
            response = "Please choose: Standard, Deluxe, or Presidential."
    ...
    # STEP 3: CONFIRM -> Ask EXIT
        ...
        if any(x in text for x in ["yes", "confirm", "ok", "sure", "authorize"]):
            ...
            response = "Authorization Verified. Booking Confirmed. Would you like to Exit System or Start New?"
            success = True
        else:
            st.session_state.booking_step = 0
            response = "Authorization Denied. Resetting protocols."
            success = True
    ...
    # STEP 4: EXIT PHASE
        ...
        if "exit" in text or "close" in text or "bye" in text:
            ...
            st.session_state.booking_step = 0
            st.session_state.db['booking'] = {k: None for k in bk}
            response = "Reloading protocols. Ready for new booking."
            success = True
        else:
            response = "Please say 'Exit' to close or 'Start New' to restart."

    # --- UPDATE ANALYTICS ---
    end_time = time.time()
    st.session_state.db['analytics']['total_queries'] += 1
    st.session_state.db['analytics']['response_times'].append(round(end_time - start_time, 3))
    if success:
...
# SCENE 2: COMMAND CENTER (Main App)
    ...
    with st.sidebar:
        ...
        # TECH METRICS
        total_q = st.session_state.db['analytics']['total_queries']
        succ_q = st.session_state.db['analytics']['successful_queries']

        col1, col2 = st.columns(2)
        col1.metric("TOTAL QUERIES", total_q)
        col2.metric("SUCCESS RATE", f"{round((succ_q / total_q) * 100, 1)}%" if total_q > 0 else "0%")

        st.write("**SYSTEM LATENCY (ms)**")
