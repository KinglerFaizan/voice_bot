...
# Steps: 0=Init, 1=Guests, 2=Type, 3=View, 4=Confirm, 5=Exit
...
def process_logic(text):
    ...
    # STEP 0: INITIALIZE -> Ask GUESTS
    if step == 0:
        st.session_state.booking_step = 1
        return "Hello. I am Friday. Let's begin. Question 1: How many guests will be staying?"

    # STEP 1: GUESTS -> Ask TYPE
    elif step == 1:
        import re
    ...
    # STEP 2: TYPE -> Ask VIEW
    elif step == 2:
        if "standard" in text:
            st.session_state.db['booking']['type'] = "Standard"
            st.session_state.booking_step = 3
            return "Standard Class. Question 3: Do you prefer a City View or Garden View?"
        elif "deluxe" in text:
            st.session_state.db['booking']['type'] = "Deluxe"
            st.session_state.booking_step = 3
            return "Deluxe Class. Question 3: Do you prefer a City View or Garden View?"
        elif "presidential" in text:
            st.session_state.db['booking']['type'] = "Presidential"
            st.session_state.booking_step = 3
            return "Presidential Suite. Question 3: Do you prefer an Ocean View or Skyline?"
        return "Class unavailable. Please choose Standard, Deluxe, or Presidential."

    # STEP 3: VIEW -> Ask CONFIRM
    elif step == 3:
        ...
        # COMPILE SUMMARY IMMEDIATELY
        b = st.session_state.db['booking']
        return f"Configuration complete: {b['type']} room with {b['view']} for {b['guests']} guests. Do you authorize this transaction?"

    # STEP 4: CONFIRM -> ASK EXIT
    elif step == 4:
        if any(x in text for x in ["yes", "confirm", "ok", "sure", "authorize"]):
            st.balloons()
            st.session_state.db['booking']['confirmed'] = True
            st.session_state.booking_step = 5  # Move to Exit Phase
            return "Authorization Verified. Reservation uploaded. Goodbye. Would you like to Exit System or Start New?"
...
if not st.session_state.system_ready:
    ...
    with c2:
        ...
        if st.button("INITIATE SYSTEM BOOT", use_container_width=True):
            ...
            # Trigger Intro Voice
            intro = "System Online. I am Friday, developed by Faizan. Let's begin. Question 1: How many guests will be staying?"
