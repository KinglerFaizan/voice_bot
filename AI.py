...
st.set_page_config(
    page_title="FRIDAY | PROTOCOL V.7",
    page_icon="⚛️",
    layout="wide",
...
# Steps: 0=Init, 1=Guests, 2=Type, 3=View, 4=Name, 5=Confirm, 6=ExitChoice
...
def process_logic(text):
    ...
    if any(x in text for x in ["reset", "cancel", "abort", "stop"]):
        ...
        return "System reset. Protocols cleared. Ready for new assignment."

    # STEP 0: INITIALIZE (Handled by Boot Button mostly, but fail-safe here)
    if step == 0:
        st.session_state.booking_step = 1
        return "Hello. I am Friday. Let's begin. How many guests will be staying?"

    # STEP 1: GUESTS -> Ask TYPE (Question 1)
    elif step == 1:
        import re
        ...
        if nums:
            ...
            return f"{nums[0]} guests confirmed. Question 1: Would you like Standard, Deluxe, or Presidential Class?"
        return "I need the number of guests to proceed. Please state a number."

    # STEP 2: TYPE -> Ask VIEW (Question 2)
    elif step == 2:
        if "standard" in text:
            st.session_state.db['booking']['type'] = "Standard"
            st.session_state.booking_step = 3
            return "Standard Class. Question 2: Do you prefer a City View or Garden View?"
        elif "deluxe" in text:
            st.session_state.db['booking']['type'] = "Deluxe"
            st.session_state.booking_step = 3
            return "Deluxe Class. Question 2: Do you prefer a City View or Garden View?"
        elif "presidential" in text:
            st.session_state.db['booking']['type'] = "Presidential"
            st.session_state.booking_step = 3
            return "Presidential Suite. Question 2: Do you prefer an Ocean View or Skyline?"
        return "Class unavailable. Please choose Standard, Deluxe, or Presidential."

    # STEP 3: VIEW -> Ask NAME (Question 3)
    elif step == 3:
        ...
        return f"{view_sel} recorded. Question 3: Under what name should I book this?"

    # STEP 4: NAME -> Ask CONFIRM
    elif step == 4:
        st.session_state.db['booking']['name'] = text.title()
        st.session_state.booking_step = 5
        b = st.session_state.db['booking']
        return f"Reservation compiled for {b['name']}: {b['type']} room, {b['view']}, {b['guests']} guests. Do you authorize this transaction?"

    # STEP 5: CONFIRM -> FINISH -> ASK EXIT
    elif step == 5:
        if any(x in text for x in ["yes", "confirm", "ok", "sure", "authorize"]):
            st.balloons()
            st.session_state.db['booking']['confirmed'] = True
            st.session_state.booking_step = 6  # Move to Exit Phase
            return "Authorization Verified. Booking Confirmed. Goodbye. Would you like to Exit System or Start New?"
    ...
    # STEP 6: EXIT PHASE
        ...
        if "exit" in text or "close" in text or "bye" in text:
            st.session_state.system_ready = False  # Shut down
            st.rerun()
...
if not st.session_state.system_ready:
    ...
    with c2:
        ...
        if st.button("INITIATE SYSTEM BOOT", use_container_width=True):
            ...
            # Trigger Intro Voice (HELLO & GUESTS)
            intro = "Hello. I am Friday. I am ready to assist you. How many guests will be staying?"
            st.session_state.db['history'].append({"role": "assistant", "content": intro})
            st.session_state.audio_buffer = intro
            st.session_state.booking_step = 1  # Set to 1 so next logic checks for Guest Number
            st.rerun()
